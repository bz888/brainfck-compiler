...
(function (app) {
  ...
  var Brainfuck = /** @class */ (function () {
    function Brainfuck (code, text) {
      ...
      for (var i = 0; i < code.length; i++) {
        ...
        switch (code.charAt(i)) {
          ...
          case ']':
            ...
            if (j === undefined) { throw new RangeError('Mismatched brackets (extra right bracket)') }
      ...
      if (openBracketIndexes.length > 0) { throw new RangeError('Mismatched brackets (extra left bracket)') }
      // Set buttons
    ...
    Brainfuck.prototype.run = function () {
      var _this = this
      if (this.runTimeout != -1) { throw new Error('Assertion error') }
      var startTime = Date.now()
      for (var i = 0; i < this.runIterations; i++) { this.step() }
      var elapsedTime = Date.now() - startTime
      if (elapsedTime <= 0) { this.runIterations *= 2 } else { this.runIterations *= Brainfuck.TARGET_RUN_TIME / elapsedTime }
      this.showState()
      if (!this.isHalted()) {
    ...
    Brainfuck.prototype.setInstructionIndex = function (newIndex) {
      if (!(newIndex >= 0 && newIndex <= this.instructions.length)) { throw new RangeError('Invalid instruction index') }
      this.instructionIndex = newIndex
    }
    Brainfuck.prototype.addMemoryIndex = function (delta) {
      var newIndex = this.memoryIndex + delta
      if (newIndex < 0) { throw new RangeError('Negative memory index') }
      this.memoryIndex = newIndex
    }
    Brainfuck.prototype.addMemoryValue = function (delta) {
      while (this.memoryIndex >= this.memory.length) { this.memory.push(0) }
    ...
    Brainfuck.prototype.readInput = function () {
      ...
      if (this.inputIndex < this.inputText.length) {
        ...
        if (val > 0xFF) { throw new Error('Input has character code greater than 255') }
        this.inputIndex++
      }
    ...
    Brainfuck.prototype.showState = function () {
              ...
              'Index = ' + addSeparators(this.memoryIndex)
      {
        ...
        while (this.memoryIndex < this.memoryViewIndex && this.memoryViewIndex >= MEMORY_VIEW_STEP) { this.memoryViewIndex -= MEMORY_VIEW_STEP }
        while (this.memoryIndex >= this.memoryViewIndex + Brainfuck.MEMORY_VIEW_WINDOW) { this.memoryViewIndex += MEMORY_VIEW_STEP }
        this.showMemoryView()
      }
      ...
      }
      {
        ...
        if (outputText === null) { throw new Error('Assertion error') }
    ...
    Brainfuck.prototype.showMemoryView = function () {
      ...
      for (var i = 0; i < Brainfuck.MEMORY_VIEW_WINDOW; i++) {
        ...
        if (index == this.memoryIndex) { tr.classList.add('active') }
  ...
  var BeginLoop = /** @class */ (function (_super) {
    ...
    BeginLoop.prototype.execute = function (bf) {
      if (bf.isMemoryZero()) { bf.setInstructionIndex(this.exitIndex) }
    }
    return BeginLoop
  }(Instruction))
  var EndLoop = /** @class */ (function (_super) {
    ...
    EndLoop.prototype.execute = function (bf) {
      if (!bf.isMemoryZero()) { bf.setInstructionIndex(this.enterIndex) }
    }
    return EndLoop
  }(Instruction))
  /* ---- Utility functions ---- */
  function addSeparators (val) {
    ...
    for (var i = result.length - 3; i > 0; i -= 3) { result = result.substring(0, i) + '\u00A0' + result.substring(i) } // Non-breaking space
    return result
  }
  ...
  function queryElem (query, type) {
    ...
    if (result instanceof type) { return result } else if (result === null) { throw new Error('Element not found') } else { throw new TypeError('Invalid element type') }
  }
